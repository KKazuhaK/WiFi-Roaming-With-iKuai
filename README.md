...
# 2. 凑一个工作目录: docker-compose.yml + .env + portal/ 必须同一级
#    (直接在仓库根用也行, 但部署机上通常单独建 /opt/wifi-portal/)
sudo mkdir -p /opt/wifi-portal && sudo chown $USER:$USER /opt/wifi-portal
...
# 4. 填关键变量 (其余按 .env.example 注释填), 最少这几个:
...
`COMPOSE_PROFILES` 留空即模式 A, 容器内部 `LISTEN_ADDR=0.0.0.0:28080`, 宿主端口只映射到 `127.0.0.1:28080`, 由你自己的 Nginx / Caddy 终结 TLS 反代过来。
...
| `CLIENT_ID` | App Registration 的应用程序(客户端) ID |
...
example.com          # 覆盖 wifi.login.example.com
...
本项目以 **GNU AGPL-3.0** 授权, 全文见 [`LICENSE`](./LICENSE)。

你可以自由使用、修改、再分发本项目; 但若把修改版作为网络服务对外提供 (而非仅本地运行), 须向使用该服务的用户提供对应的完整源码。

打包依赖 (`coreos/go-oidc`、`golang.org/x/oauth2`、`go-jose/go-jose`) 为 Apache-2.0 / BSD-3-Clause 许可, 与 AGPL-3.0 兼容。
