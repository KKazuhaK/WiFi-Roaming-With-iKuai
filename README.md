...
│   ├── active_devices.go      # 内存里维护"最近放行设备"映射, 给活跃设备面板用
...
`/admin` 页面顶部是**概览看板** (8 张卡片): 24h 登录数、7d 登录数、7d 失败占比、当前活跃设备、有效访客码、封禁 IP / MAC、全部访客码总数。下面是六个 tab:
...
**活跃设备**
- 列出最近放行过且未超出 `ACTIVE_DEVICE_WINDOW` 的所有设备 (MAC / 用户 / IP / 方式 / 首次放行 / 最近活跃)
- 一键封禁 (MAC) 或从面板移除
- **不持久化, 容器重启清零** (数据随使用自然重建)
...
- **永远不记录敏感数据**: 不写完整访客码 / 密码 / token, 只记码尾 4 位
...
**事件日志持久化**: 所有登录事件 + admin 审计写到 JSONL 文件 (一行一条 JSON),
默认 `/data/events.jsonl`, 保留期 `EVENT_LOG_RETENTION_DAYS=7` 天 (后台 gc 每小时
跑一次, Prune 时重写整个文件)。`tail -f /data/events.jsonl` 可实时看流。
留空 `EVENT_LOG_PATH` = 纯内存模式, 重启清零。

```
EVENT_LOG_PATH=/data/events.jsonl   # 空 = 纯内存
EVENT_LOG_RETENTION_DAYS=7
ACTIVE_DEVICE_WINDOW=24h            # 活跃设备超过此时长不活动后从面板剔除
```

事件 schema (JSONL 每行一条):
```json
...
{"time":"2026-04-24T14:31:04Z","kind":"admin_action","subject":"me@kazuha.org",
 "result":"success","method":"admin","detail":"add code=…1234"}
```

写盘失败只 log 不阻塞业务路径 — 事件日志不是关键路径, 丢一条也不影响放行。
...
EVENT_LOG_PATH=/data/events.jsonl # 事件日志 JSONL (留空 = 纯内存)
EVENT_LOG_RETENTION_DAYS=7        # 事件保留期
ACTIVE_DEVICE_WINDOW=24h          # 活跃设备面板剔除窗口
```
...
**事件日志 / 活跃设备 / Dashboard** 的运营用法见上面"Admin 能做什么"段。
...
- [x] MAC 永久封禁列表 (`DENYLIST_PATH`, 管理员在 `/admin` 维护, 支持 CSV 导入导出)
- [x] 账号枚举防护 (`/auth/start` → opaque token → `/auth/proceed` 中转)
- [x] `robots.txt` 拒爬 + 模板 `<meta robots noindex nofollow>`
- [x] 结构化事件日志 + admin 审计 (`EVENT_LOG_PATH`, JSONL, 默认保留 7 天)
- [x] 活跃设备看板 (内存; `ACTIVE_DEVICE_WINDOW=24h` 内剔除)
