...
# 构建
docker build -t ghcr.io/kkazuhak/wifi-portal:latest ./portal
docker build -t wifi-portal-caddy:latest -f deploy/Dockerfile.caddy deploy/

# 导成一个 tarball (包含两个镜像, 一个文件传一次省事)
docker save -o /tmp/wifi-portal-stack.tar \
    ghcr.io/kkazuhak/wifi-portal:latest \
    wifi-portal-caddy:latest
...
4. 导入完应该能在列表看到 `ghcr.io/kkazuhak/wifi-portal:latest` 和 `wifi-portal-caddy:latest` 两条
...
3. 下一步 → 会解析出 `portal` 和 `caddy` 两个服务, 镜像都用 `ghcr.io/kkazuhak/...` (UI 确认能匹配到 Step 3 导入的那两个)
...
**Portal 容器**:
- 镜像: `ghcr.io/kkazuhak/wifi-portal:latest`
- 名称: `wifi-portal`
- 重启策略: `unless-stopped`
- 端口: 不发布 (Caddy 内网访问)
...
```
静态 DNS: wifi.login.example.com → <Portal 宿主 LAN IP>
自定义认证 URL: https://wifi.login.example.com:28081/portal
```
