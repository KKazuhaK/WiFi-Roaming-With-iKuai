...
公网 VPS 方案下 `wifi.login.kazuhahub.com` 必须公网可达, 所以攻击面包括:
...
/ `kazuhahub.com` (只给这一个 zone 权限) → Continue → Create Token → 复制 token 字符串
...
| `PORTAL_HOSTNAME` | `wifi.login.kazuhahub.com` (一般不变) |
| `TENANT_ID` / `CLIENT_ID` / `CLIENT_SECRET` | Entra App Registration, 和 VPS 方案一样 |
| `IKUAI_APPKEY` | iKuai 云控制台生成 (每个站点可能 appkey 不同) |
| `PUBLIC_URL` | `https://wifi.login.kazuhahub.com`, 必须和 PORTAL_HOSTNAME 一致 |
...
预期 Caddy 日志里有:
```
{"level":"info","msg":"obtaining certificate","identifiers":["wifi.login.kazuhahub.com"]}
...
curl -v --resolve wifi.login.kazuhahub.com:443:<LAN_IP_OF_BOX> \
  https://wifi.login.kazuhahub.com/healthz
# 预期: 证书有效 + 响应 "ok"
```
...
```
wifi.login.kazuhahub.com  →  <LAN 盒子 IP, 如 192.168.1.50>
```
...
Phase 4 的 iKuai 自定义认证 URL 保持 `https://wifi.login.kazuhahub.com/portal`,
设备 captive 时 302 到这个 URL — 通过上一步的 DNS 劫持, 实际打的是 LAN 盒子。

### 8. 免认证白名单可以收窄

原来 VPS 方案需要的 `wifi.login.kazuhahub.com` 这一条白名单**可以去掉** (设备根本不走公网)。
...
### 设备打开 `wifi.login.kazuhahub.com` 证书报错

- iKuai DNS 劫持生效了吗? `nslookup wifi.login.kazuhahub.com` 在设备上应该返 LAN IP
